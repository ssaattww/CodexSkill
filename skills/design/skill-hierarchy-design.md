...
└─ skill-authoring-wrapper [親が実行]
   ...
   └─ `skills/design/skill-hierarchy-design.md` を更新する
```
...
9. `agents/openai.yaml` を最終的な skill 意図に合わせて更新する。
10. skill inventory、呼び出しツリー、役割、契約一覧のどれかが変わるなら `skills/design/skill-hierarchy-design.md` を更新する。
...
| `skill-authoring-wrapper` | skill purpose、target location、new/update distinction、repo standards | repo-standard local skill、aligned `agents/openai.yaml`、updated hierarchy design | built-in `skill-creator` 出力が repo 標準に補正されている |
