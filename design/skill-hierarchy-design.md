...
| Skill名 | 役割 | 実行方式 |
| --- | --- | --- |
| `review-enforcer` | task 完了前に必ずレビューを通し、セッション内で原則同じ reviewer sub-agent を使い、review report が残るまで完了扱いにしない。 | `親が実行` |
...
| Skill名 | 入力 | 出力 | 完了条件 |
| --- | --- | --- | --- |
| `review-enforcer` | task-scoped diff、review scope、workspace context、validation context、session reviewer、適用するレビュー指針 | review report、findings または no findings、disposition、reviewer reuse decision | review evidence が report に残り disposition 済み |
