...
└─ report-output-manager [親が実行, handover を report 化する場合のみ]
...
| `handover-memo-writer` | 別チャットへ移る前に、背景、経緯、決定事項、未解決事項、次の依頼文まで含む resume-ready な handover memo を作る。必要なら report 化も行う。 | `親が実行` |
...
| `handover-memo-writer` | current chat history、agreed constraints、repo/report/git state、user format request | next-chat ready handover memo、必要なら handover report | 新しい chat が旧会話なしでも再開可能な handover が完成している |
